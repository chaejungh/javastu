...
```
자료형을 선언하면 매번 새로운 자료가 생성되기때문에 두 배열 자료는 다르다
컴퓨터는 기본적으로 자료형을 비교할때 자료형 내부의 데이터(필드를) 하나씩 비교해서 같은지 반환하지 않는다.
---

**4.** 문자열 비교 시 `==`와 `.equals()`의 차이는 무엇인가?
== 은 주소값을 비교하는것, equals 는 자료형의 내부 필드가 모두 같은지 하나씩 확인하는 함수이다.
---
...
String s2 = "hello";"hello"가 메모리에 있는지 확인 => 있어서 (@123)참조
