...
class for if import 등등 => 자바 구현을 위한 약속된 표기 => 예약어
⸻
...
맨 위에 위치해야합니다. 자식클래스의 생성자의 초기화보다 앞에 위치해야함.
⸻

	5.	부모 클래스에 기본 생성자가 없는 경우, 자식 클래스는 어떤 동작을 해야 하나요?
자식 클래스에서 super를 사용해 부모 생성자를 호출해야함
⸻
...
메서드를 다른 매개변수를 받으며 다시 정의하는것을 메서드 오버로딩이라고 합니다
⸻
...
⸻
	...
	두객체의 필드가 같은지 확인하ㅣㄱ 위해ㅐ 재정의해야함
	//컴퓨터 : 두 객체가 다른주소면 다른객체(==)
	//사람 : 두 객체의 필드가 같은 객체 아닌가?를 구현하는 메서드
객체가 같은지 확인.
⸻

	10.	부모와 자식 클래스가 동일한 이름의 필드를 가질 경우, this.name과 super.name은 무엇을 의미하나요?
super.name은 부모의 name변수를 의미하고 this.name은 해당 객체의 name을 의미한다.
⸻
...
부모 생성자를 호출하지않았고, 양식도 맞지않는다(컴파일 오류)
⸻
...
패키지명.클래스@해시코드/ Object.toString()이 출력됩니다.
⸻

	17.	오버라이딩(Overriding)과 오버로딩(Overloading)의 차이를 간단히 설명해보세요.
	다형성(1개인데 역할이 여러개인 것, 유연한 것)의 일부로,
오버라이딩은 부모 메서드를 상속한다는 것을 명시적으로 표현한 것이고(자식이 재정의하는것)
오버로딩은 동일한 이름의 메서드를 매개변수 값을 바꾸어 재정의하는 것을 말합니다.
⸻
...
자바는 다양한 타입의 객체를 동일한 타입으로 다루는 것이 필요하고,
타입의 기본형태를 제공하며, 공통된 함수 사용을 위해.

모범 답안: 공통된 메서드 사용을 위해 (toString, equals, hashCode 등), 다형성과 범용 처리를 지원. 설계의 유연성
⸻
...
부모 클래스인 Parent에는 생성자에 매개변수가 존재하지만 자식 클래스인 Child에는 매개변수가 존재하지않는 생성자이다. Child클래스는 부모 클래스와 동일하게 매개변수를 가져야하지만 그렇지않으므로 오류가 발생한다.
⸻
...
③ 생성자에서 super() 호출은 생략 가능하다. // default일때 super()<<- 이 안에 아무것도 없는게 디폴트일때
...
Object의 toString()을 재정의하지 않아서 toString은 패키지.클래스@해시코드를 반환합니다. 이는 해당 코드에서 얻으려고 하는 Book의 타이틀과는 다른 결과가 출력됩니다
"자바"라는 출력결과를 원한다면
toString을 오버라이딩하여
toString(){
    return "Book title: "+this.title+"}";  처럼 출력 형식을 정해줘야합니다
}
⸻

27. @Override 어노테이션을 사용하는 이유를 설명하고, 실수로 오타가 날 경우 어떤 장점이 있는지 예시와 함께 설명하세요.

컴파일시에 이 메서드는 부모 메서드를 오버라이딩한 메서드인걸 컴퓨터에게 알려줍니다.
만약
class Book {
    String title = "자바";
    @Override
   String totring(){
    return "Book title: "+title"}";  처럼 출력 형식을 정해줘야합니다
}
}
...
의 경우에 Object.toString()을 재정의 하려고 했지만 메서드 이름이 totring으로 오타가 났습니다 이러한 경우에
 컴파일 에러가 뜨며 개발자의 실수를 줄여줄 수 있다는 장점이 있습니다.
⸻
...
this()는 같은 클래스의 다른 생성자를 호출하는 것이며, super()와 this()는 동시에 쓸 수 없음
class A {
    A(int x) {}
}
...
super는 부모 클래스의 객체를 나타내는 것이고 this는 해당클래스의 객체를 나타내는 것입니다.
⸻
좋습니다!
...
class Studentt extends Member{
    String school;
    Studentt(String name, String school){
        super(name);
        this.school = school;
    }
...
public static void main(String[] args) {
        Studentt st = new Studentt("홍길동","코스모스 고등학교");
...
질문: 왜 false가 나올까요? 두 객체값을 비교하는 것이기 때문에 값이 같아도 false가 나옵니다
