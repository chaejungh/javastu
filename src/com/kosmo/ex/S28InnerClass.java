...
public class S28InnerClass {
    ...
    class Nested{// 중첩 클래스
        int a=10;
        void mult(){
            this.a++;// new Nested().a          //중첩클래스가 객체일때는 무저건 외부클래스가 객체일때기 때문
            S28InnerClass.this.a++; //new S28InnerClass().a
        }
    }
//  외부클래스 내부에서 특정 타입(외부클래스에서만 사용하는..)이 필요할때 작성

    void sum(){

    }

//    static : 외부임. 스태틱은 (S28InnerClass의 외부)외부이기 때문
    public static void main(String[] args) {
        ++new S28InnerClass().a;
        //this.a; //S28InnerClass의 내부에 작성된 main이지만 static은 독립적이기 때문에 필드 접근 불가
        S28InnerClass.Nested n = new S28InnerClass().new Nested(); //객체로 만들고 객체 안에서 또 만들기
    }
    /*
...
사용 목적	보통 외부 클래스의 기능을 보조하기 위해 사용됨
