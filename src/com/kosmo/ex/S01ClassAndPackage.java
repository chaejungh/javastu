...
(서술형)
Ans:L01Hello 클래스 --> main 함수 --> 1024 출력
...
A. 자바 코드를 컴파일하는 도구이다.
B. 자바 코드를 실행하는 환경이다.
C. 자바 코드를 작성하는 IDE이다. ==> 인텔리제이, 이클립스
...
(서술형)
패키지를 사용하지않으면 중복되는 클래스들이 발생한다 그것을 방지하기위해 패키지를 사용한다.
(클래스 이름을 유일하게 만들기위해)<= 디렉토리 구조를 이용함.

모범답안: 패키지는 클래스 이름이 충돌하지 않도록 이름 공간(namespace)을 제공하고, 관련된 클래스를 논리적으로 그룹화하기 위해
사용됩니다.
...
Ans:9 (Math.pow는 실수반환이기에    9.0이 더 맞는 답)
