...
//객체에 대한 중간단계 미완성 설계
abstract class A{
    abstract (접근지정자,private 제외) int a();
}  new A();  X
...
//정수(int) 타입의 최대값(고정값) Integer.MAX.VALUE;; =>최대값, MATH.PI

interface A{
    //void a(){} // 오류
    ( public static final)int i =10; //자동으로 static(클래스 변수) 필드로 바뀜, 인터페이스는 필드를 가질 수 없다
...
//extends: 부모도 객체가 되어서 자식에게 포함된다. new Child(); <= extends 한 부모객체가 포함
//implements : 부모의 타입만 사용(부모는 객체가 되지 않음, 다중구현이 가능)
//인터페이스가 다중구현이 가능한 이유 : 필드가 없이 추상메서드로만 이뤄진 타입이기때믄.
interface C implements A,B{
    void sum(){
    super.i++; // 다중구현시 부모의 필드가 누구의 것인지 알 수 없음(우선순위가 없기때문)
    }
    @Override
    public void a(){// A a(), B a() => 둘다 구현이 안되어 있어서 상관 없다.
