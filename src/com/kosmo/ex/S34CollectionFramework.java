...
2.2 Set
	•	순서와 길이가 없음, 중복 불허
	•	대표 클래스: HashSet, LinkedHashSet(순서는 없지만 이전자료가 다음 자료를 참조), TreeSet
	•	주요 특징:
	•	HashSet: 순서 없음
...
D. list.clear(1)
B
	6.	ArrayList에 존재하지 않는 값(오브젝트)을 삭제하려고 하면?

A. 오류 발생
B. 프로그램 종료
C. 아무 일도 일어나지 않음 //false 반환
D. 자동으로 삽입됨
A XXXX   C
⸻
...
B. 오류 발생
C. 한 번만 저장된다
D. 마지막 것만 저장된다  //검사하고 동일한 값이면 저장하지않음
D XXX C
...
문제:
HashSet<String>을 만들어 "apple", "banana", "apple", ""를 추가하고, 전체 요소를 출력하시오.
...
그 후 "user1"의 비밀번호를 출력하시오.
   HashMap<String,String> idd = new HashMap<>();

        idd.put("user1","1234");

        idd.put("user2","abcd");

        for(String key : idd.keySet()){
          Object val = idd.get(key);
...
HashMap<String, String> hm = new HashMap<>();
    hm.put("name", "Alice");
    hm.put("email", "alice@example.com");
