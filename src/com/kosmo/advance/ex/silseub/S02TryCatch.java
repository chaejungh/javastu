...
2. 예외의 종류
	...
	•	예외 위임으로 컴파일러가 예외를 알도록 하는 것: IOException, SQLException
...
try {
    ...
    @와 인터넷으로 접속; => @의 인터넷이 끊어지면 IO 오류 발생

    @에게 파일 보내기 => @의 주소가 잘못되면 IO오류 발생

    String s = null;
    s.length();
} catch (FileNotFindException e) { // 파일을 찾았는데 없을때
    System.out.println("널 예외");
} catch (IOException e) {// 통신 중일떄
...
5. 예외 객체의 주요 메서드
	•	e.getMessage() : 예외 메시지 :  throw new Exception("hi") => "hi"
	•	e.printStackTrace() : 예외 발생 위치 추적 출력 == System.out.print(e) + 빨간 로그
