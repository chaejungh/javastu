...
- 스트림의 방향성
	•	입력(Input) 스트림
        외부(파일, 키보드, 네트워크 등)에서 데이터를 읽어와 프로그램 내부로 가져오는 흐름
            ...
            •	OS의 파일 시스텐에서 자바 어플이 데이터 읽기
            •	서버로부터 응답 받기
    •	출력(Output) 스트림
        프로그램 내부에서 생성된 데이터를 외부(파일, 화면, 네트워크 등)로 보내는 흐름
            예:
            •	파일에 데이터 저장
            •	콘솔에 출력 System.out.println("안녕") // out == OutputStream
            •	네트워크를 통해 데이터 전송
...
- 스트림은 단방향이다
	...
	•	Reader / Writer
	•	양방향 통신을 하려면 두 개의 스트림을 사용해야 함 // input output stream 두개 다 만들어여함
	•	예: 소켓 통신에서는 getInputStream()과 getOutputStream()을 따로 사용

⸻

- 스트림의 핵심 특징 요약
	•	순차적 처리(Iterator): 데이터를 한 번에 하나씩 읽고 씀 (대용량 처리에 적합)
...
4-1. FileWriter  (문자 스트림)
    FileWriter writer = null; 형제 블럭이라 finally 에서 writer를 못불러옴
    try () {
